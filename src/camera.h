...
namespace ecto_gl
{
  ...
  public:
    ...
    inline
    Frame(const Eigen::Vector3f& pos = Eigen::Vector3f::Zero(), const Eigen::Quaternionf& o = Eigen::Quaternionf())
        :
          orientation(o),
          position(pos)
    {
    ...
    Frame
    lerp(float alpha, const Frame& other) const
    {
  ...
  public:
    ...
    void
    setViewport(uint offsetx, uint offsety, uint width, uint height);
    void
    setViewport(uint width, uint height);
    ...
    void
    setPosition(const Eigen::Vector3f& pos);
    ...
    void
    setOrientation(const Eigen::Quaternionf& q);
    inline const Eigen::Quaternionf&
    ...
    void
    setTarget(const Eigen::Vector3f& target);
    inline const Eigen::Vector3f&
    target(void)
    {
      return mTarget;
    ...
    void
    rotateAroundTarget(const Eigen::Quaternionf& q);
    void
    localRotate(const Eigen::Quaternionf& q);
    void
    zoom(float d);
    ...
    Eigen::Vector3f
    unProject(const Eigen::Vector2f& uv, float depth) const;

  protected:
    void
    updateViewMatrix(void) const;
    void
    updateProjectionMatrix(void) const;
