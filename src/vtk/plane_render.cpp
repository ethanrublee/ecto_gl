...
void
vtk_to_K(vtkRenderWindow* renderWindow, vtkCamera* cam, cv::Mat& K, cv::Mat& R, cv::Mat& T);

void
grab_frame(vtkRenderWindow* renderWindow, cv::Mat& image, cv::Mat& depth, cv::Mat& mask, cv::Mat& K, cv::Mat& R,
           cv::Mat& T);
void
callback(vtkObject* caller, long unsigned int eventId, void* clientData, void* callData);
           ...
           cv::Mat& T)
{
  ...
  vtk_to_K(renderWindow, camera, K, R, T);
  {
...
ECTO_CELL(ecto_sim, PlanarSim, "FiducialSim", "Simulates a view of a fiducial marker.");
#else
int
